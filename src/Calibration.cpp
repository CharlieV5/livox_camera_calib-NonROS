...
bool Calibration::loadCameraConfig(const std::string &camera_file)
{
	...
	{
		std::cerr << "Failed to open camera settings file at " << camera_file << std::endl;
		return false;
	}

	std::string model = cameraSettings["Camera.model"];
	if (model == "pinhole")
		m_cam_model = 0;
	...
	{
		std::cerr << "Failed to read 'CameraMat' as a matrix from camera settings file." << std::endl;
		return false;
	}

	// 确保 CameraMat 是 3x3 矩阵
	if (camera_matrix_.rows != 3 || camera_matrix_.cols != 3)
	{
		std::cerr << "'CameraMat' is not a 3x3 matrix." << std::endl;
		return false;
	}
	...
	{
		std::cerr << "Failed to read 'DistCoeffs' as a matrix from camera settings file." << std::endl;
		return false;
	}
	...
	{
		dist_coeffs_ = dist_coeffs_.t(); // 转换为列向量
	}
	else if (dist_coeffs_.cols != 1)
	{
		std::cerr << "'DistCoeffs' is not a column vector." << std::endl;
		return false;
	}
	...
	pcl::PointCloud<pcl::PointXYZRGBL>::Ptr &color_cloud)
{
	...
	for (size_t i = 0; i < pts_2d.size(); i++)
	{
			...
			pts_2d[i].y < image_rows)
		{
			...
			p.label = (std::uint32_t)(intensities[i]);
	...
	std::vector<VPnPData> &pnp_list)
{
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr tree_cloud_lidar(new pcl::PointCloud<pcl::PointXYZ>);
...
//      Eigen::AngleAxisd(extrinsic_params[1], Eigen::Vector3d::UnitY()) *
//      Eigen::AngleAxisd(extrinsic_params[2], Eigen::Vector3d::UnitX());
//  Eigen::Vector3d transation(extrinsic_params[3], extrinsic_params[4], extrinsic_params[5]);
//
