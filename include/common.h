...
static std::vector<fs::path> get_files_by_extensions(const std::string& dir, const std::vector<std::string>& extensions) 
{
	std::vector<fs::path> matched_files;
    fs::path current_dir(dir);

	// 预处理扩展名：统一小写并添加点前缀 
	std::vector<std::string> processed_exts;
	for (const auto& ext : extensions) 
    {
		std::string processed = ext;
		if (!ext.empty() && ext[0] != '.') 
            processed = "." + ext;

        // 转换为小写
		std::transform(processed.begin(), processed.end(), processed.begin(), [](unsigned char c) { return std::tolower(c); });
		processed_exts.push_back(processed);
	}
	...
	for (const auto& entry : fs::directory_iterator(current_dir))
    {
		if (entry.is_regular_file()) 
        {
			...
			if (std::find(processed_exts.begin(), processed_exts.end(), file_ext) != processed_exts.end()) 
            {
...
static void mapJet(double v, double vmin, double vmax, uint8_t &r, uint8_t &g, uint8_t &b)
{
  r = 255;
  g = 255;
  b = 255;
